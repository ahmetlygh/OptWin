...
import { SearchIcon, XIcon, CheckCircleIcon } from "../shared/Icons";
...
export function SearchBar() {
    ...
    return (
        <div className="relative w-full max-w-xl mx-auto group animate-fade-in-up">
            ...
            <input
                ...
                className="block w-full pl-11 pr-36 py-3.5 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl text-[var(--text-primary)] placeholder-[var(--text-secondary)] focus:ring-2 focus:ring-[var(--accent-color)]/50 focus:border-[var(--accent-color)] shadow-sm transition-all duration-300"
            ...
            <div className="absolute inset-y-0 right-0 pr-3 flex items-center justify-end gap-2 pointer-events-none">
                ...
                <div
                    className={`pointer-events-auto flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all duration-300 ${selectedCount > 0 ? 'bg-[var(--accent-color)]/10 border-[var(--accent-color)]/30 text-[var(--accent-color)] shadow-[0_0_10px_rgba(107,91,230,0.2)]' : 'bg-[var(--bg-color)] border-[var(--border-color)] text-[var(--text-secondary)]'}`}
                >
                    {selectedCount > 0 && <CheckCircleIcon size={14} className="animate-pop-in" />}
                    <span className="text-xs font-bold whitespace-nowrap">
                        {selectedCount} {lang === "tr" ? "seçildi" : "selected"}
                    </span>
                </div>
            </div>
        </div>
