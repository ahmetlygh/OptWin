...
export function RestorePointModal() {
    ...
    return (
        ...
        >
            <div
                className={`w-full max-w-lg bg-(--card-bg) border border-(--border-color) rounded-3xl p-8 md:p-10 shadow-2xl relative overflow-hidden ${isVisible ? 'modal-content-enter' : phase === 'exiting' ? 'modal-content-exit' : ''}`}
                onClick={(e) => e.stopPropagation()}
            >
                <div className="absolute top-0 right-0 w-40 h-40 bg-(--accent-color)/10 rounded-full blur-[50px] pointer-events-none"></div>
                ...
                <div className="flex flex-col items-center text-center gap-6 relative z-10">
                    <div className="size-20 rounded-full bg-(--accent-color)/20 text-(--accent-color) flex items-center justify-center mb-2 shadow-[0_0_20px_rgba(108,92,231,0.3)] border border-(--accent-color)/30 animate-pop-in">
                        <RestoreIcon size={36} />
                    </div>

                    <h3 className="text-2xl font-extrabold text-(--text-primary) tracking-tight animate-fade-in-up" style={{ animationDelay: "0.05s" }}>
                        {t["restore.title"]}
                    </h3>

                    <p className="text-(--text-secondary) leading-relaxed text-base pb-4 animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
                    ...
                    <div className="flex flex-col sm:flex-row w-full gap-4 animate-fade-in-up" style={{ animationDelay: "0.15s" }}>
                        ...
                        <button
                            ...
                            disabled={isGenerating}
                            className="flex-1 py-4 bg-transparent border-2 border-(--border-color) hover:border-(--text-secondary) text-(--text-primary) font-bold text-lg rounded-xl flex items-center justify-center gap-2 disabled:opacity-50 transition-all duration-300 hover:-translate-y-0.5"
                        >
                            {t["restore.noSkip"]}
                        </button>
                    </div>
