...
export function ScriptOverlay({ serverSettings = {} }: ScriptOverlayProps) {
    ...
    return (
        ...
        >
            <div
                className={`w-full max-w-6xl bg-(--card-bg) border border-(--border-color) rounded-3xl overflow-hidden shadow-2xl shadow-(--accent-color)/10 relative flex flex-col md:flex-row h-[85vh] md:h-[80vh] ${isVisible ? 'modal-content-enter' : phase === 'exiting' ? 'modal-content-exit' : ''}`}
            ...
            >
                ...
                <div className="w-full md:w-[28%] bg-(--bg-color) p-6 md:p-8 flex flex-col justify-between border-b md:border-b-0 md:border-r border-(--border-color) relative shrink-0">
                    <div className="absolute inset-0 bg-linear-to-b from-(--accent-color)/5 to-transparent pointer-events-none"></div>
                    <div className="space-y-8 relative z-10">
                        <div className="animate-fade-in-up">
                            <h2 className="text-3xl font-black text-(--text-primary) mb-2 tracking-tight">{t["script.ready"]}</h2>
                        ...
                        <div className="space-y-4 animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
                            <h3 className="text-xl font-bold text-(--text-primary) flex items-center gap-2">
                                <BookOpenIcon size={18} className="text-(--accent-color)" />
                                {t["script.howToUse"]}
                            </h3>
                            <ul className="space-y-4 text-(--text-secondary) text-sm stagger-children">
                                <li className="flex gap-3">
                                    <span className="shrink-0 size-6 rounded-full bg-(--accent-color)/20 text-(--accent-color) font-bold flex items-center justify-center">1</span>
                                    <span>{t["script.step1"]}</span>
                                </li>
                                <li className="flex gap-3">
                                    <span className="shrink-0 size-6 rounded-full bg-(--accent-color)/20 text-(--accent-color) font-bold flex items-center justify-center">2</span>
                                    <span>{t["script.step2"]} <b className="text-(--text-primary)">{t["script.runAsAdmin"]}</b>.</span>
                                </li>
                                <li className="flex gap-3">
                                    <span className="shrink-0 size-6 rounded-full bg-(--accent-color)/20 text-(--accent-color) font-bold flex items-center justify-center">3</span>
                        ...
                        <div className="md:hidden animate-fade-in-up" style={{ animationDelay: "0.15s" }}>
                            <button
                                ...
                                className="w-full flex items-center justify-center gap-2 h-12 bg-(--accent-color) hover:bg-(--accent-hover) text-white font-bold rounded-xl shadow-[0_5px_15px_rgba(168,85,247,0.3)] transition-all duration-200 disabled:opacity-70"
                            >
                    ...
                    <div className="pt-6 border-t border-(--border-color) hidden md:block animate-fade-in-up relative z-10" style={{ animationDelay: "0.2s" }}>
                        <div className="flex flex-col gap-3">
                            <button
                                onClick={handleDownload}
                                disabled={isDownloading}
                                className="w-full flex items-center justify-center gap-2 h-12 bg-(--accent-color) hover:bg-(--accent-hover) text-white font-bold rounded-xl shadow-[0_5px_15px_rgba(168,85,247,0.3)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_20px_rgba(168,85,247,0.4)] disabled:opacity-70"
                            >
                                {isDownloading ? <LoaderIcon size={18} className="animate-spin" /> : <DownloadIcon size={18} />} {t["script.download"]}
                            </button>
                            <button
                ...
                <div className="w-full md:w-[72%] p-4 md:p-6 flex flex-col min-h-0 bg-(--card-bg)">
                    <div className="flex items-center justify-between bg-(--bg-color)/50 border border-(--border-color) rounded-t-xl px-4 py-3 shrink-0 animate-fade-in-up" style={{ animationDelay: "0.15s" }}>
                        <div className="flex items-center gap-2 text-xs font-mono text-(--text-secondary)">
                            <MonitorCog size={14} className="text-purple-500" /> {settings.site_name || "OptWin"}.bat
                        </div>
                        <button
                            onClick={handleCopy}
                            className="flex items-center gap-2 text-xs font-bold text-(--text-primary) bg-(--text-secondary)/10 hover:bg-(--text-secondary)/20 px-3 py-1.5 rounded-lg border border-(--border-color) transition-all duration-200 hover:scale-105"
                        >
                            <CopyIcon size={14} />
                            {t["script.copy"]}
                        </button>
                ...
                {(supportVisible || supportExiting) && (
                    ...
                    >
                        <div
                            className={`w-[90%] max-w-sm bg-(--card-bg) border border-(--border-color) rounded-2xl p-6 shadow-2xl shadow-(--accent-color)/20 text-center transition-all duration-250 ${supportExiting ? "opacity-0 scale-95" : "animate-pop-in"}`}
                            onClick={(e) => e.stopPropagation()}
                        >
                            <div className="flex items-center justify-center gap-3 mb-4">
                                ...
                                <h3 className="text-lg font-black text-(--text-primary)">{t["script.supportPromptTitle"]}</h3>
                            </div>
                            ...
                            <div className="flex flex-col gap-2.5">
                                <a
                                    ...
                                    className="w-full py-3 bg-linear-to-r from-amber-500 to-orange-500 text-white font-bold text-sm rounded-xl shadow-lg shadow-amber-500/20 hover:shadow-amber-500/40 hover:-translate-y-0.5 flex items-center justify-center gap-2 transition-all duration-300"
                                >
                                    <CoffeeIcon size={16} />
                                ...
                                <button
                                    disabled
                                    className="w-full py-3 bg-(--text-secondary)/5 text-(--text-secondary)/40 font-bold text-sm rounded-xl border border-(--border-color) flex items-center justify-center gap-2 cursor-not-allowed"
                                >
                                ...
                                <button
                                    onClick={dismissSupport}
                                    className="w-full py-2.5 text-(--text-secondary) hover:text-(--text-primary) text-sm font-medium transition-colors"
                                >
