...
export function HeroStats({ totalVisits, totalScripts, totalDownloads, featuresCount }: HeroStatsProps) {
    ...
    return (
        <div className="flex items-end gap-4 sm:gap-7 animate-fade-in-up flex-wrap mt-auto" style={{ animationDelay: "0.3s" }}>
            {stats.map((stat, i) => (
                <div key={stat.labelKey} className="flex items-end gap-4 sm:gap-7">
                    ...
                    {i < stats.length - 1 && (
                        <div className="w-px h-10 sm:h-12 bg-gradient-to-b from-transparent via-[var(--accent-color)]/30 to-transparent shrink-0"></div>
                    )}
                </div>
            ))}
        </div>
