...
export default function Contact() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Basic client validation to save network
        ...
        try {
            ...
            if (data.success) {
                ...
                setStatus("error");
                fetchCaptcha(); // Refresh token on failure
            }
        } catch {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[75vh] w-full animate-fade-in-up mt-10 mb-20">
                    ...
                    <AnimatePresence mode="wait">
                        {status === "success" ? (
                            ...
                            >
                                ...
                                <p className="text-[var(--text-secondary)] mb-10 text-lg font-medium max-w-sm">
                                    {t["contact.sentDesc"]}
                                </p>
                                <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                                    <button
                                        ...
                                        className="px-8 py-3.5 rounded-2xl bg-[var(--accent-color)] text-white font-bold hover:brightness-110 active:scale-95 shadow-[0_8px_20px_rgba(108,92,231,0.25)] flex-1 sm:flex-none transition-all"
                                    >
                                        Yeni Mesaj Gönder
                                    </button>
                                    <Link
                                        href="/"
                                        className="px-8 py-3.5 rounded-2xl bg-[var(--input-bg)] border border-[var(--border-color)] text-[var(--text-primary)] font-bold hover:bg-[var(--border-color)] flex items-center justify-center gap-2 flex-1 sm:flex-none transition-all active:scale-95"
                                    >
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="flex flex-col gap-2.5">
                                        <label className="text-[12px] font-bold text-[var(--text-secondary)] uppercase tracking-[0.1em] ml-1">
                                            {t["contact.name"]}
                                        </label>
                                        <input
                                            required
                                            ...
                                            className="w-full px-5 py-4 bg-[var(--input-bg)]/50 backdrop-blur-sm border border-[var(--border-color)] hover:border-[var(--accent-color)]/30 rounded-[1.25rem] text-[var(--text-primary)] font-medium placeholder-[var(--text-secondary)]/40 focus:outline-none focus:ring-4 focus:ring-[var(--accent-color)]/10 focus:border-[var(--accent-color)]/60 transition-all shadow-sm"
                                ...
                                <div className="flex flex-col gap-2.5">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full px-5 py-4 bg-[var(--input-bg)]/50 backdrop-blur-sm border border-[var(--border-color)] hover:border-[var(--accent-color)]/30 rounded-[1.25rem] text-[var(--text-primary)] font-medium placeholder-[var(--text-secondary)]/40 focus:outline-none focus:ring-4 focus:ring-[var(--accent-color)]/10 focus:border-[var(--accent-color)]/60 transition-all resize-none shadow-sm"
                                        placeholder="Mesajınızı buraya girin... (En az 20 karakter)"
                                    />
                                </div>
                                ...
                                <div className="flex flex-col sm:flex-row items-center gap-4 bg-[var(--card-bg)] border border-[var(--border-color)] p-4 rounded-[1.25rem]">
                                    <div className="flex items-center gap-3 shrink-0">
                                        <div className="w-10 h-10 rounded-xl bg-[var(--accent-color)]/10 text-[var(--accent-color)] flex items-center justify-center font-black tracking-widest border border-[var(--accent-color)]/20 shadow-inner">
                                            ?=?
                                        </div>
                                        <div>
                                            <p className="text-[11px] font-black text-[var(--text-secondary)] uppercase tracking-widest shrink-0">İnsan Doğrulaması</p>
                                            <p className="text-[14px] font-bold text-[var(--text-primary)]">
                                                {captchaQuestion ? `${captchaQuestion}` : "Soru yükleniyor..."}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <input
                                        required
                                        type="number"
                                        disabled={status === "loading" || !captchaQuestion}
                                        value={captchaAnswer}
                                        onChange={(e) => setCaptchaAnswer(e.target.value)}
                                        className="w-full sm:w-full flex-1 px-4 py-3 bg-[var(--input-bg)]/60 border border-[var(--border-color)] hover:border-[var(--accent-color)]/40 rounded-xl text-[var(--text-primary)] font-black text-center focus:outline-none focus:ring-4 focus:ring-[var(--accent-color)]/10 focus:border-[var(--accent-color)]/70 transition-all placeholder-[var(--text-secondary)]/30"
                                        placeholder="Cevap"
                                    />
                                </div>
