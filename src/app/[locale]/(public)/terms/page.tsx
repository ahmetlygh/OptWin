...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    // Task 3: Dynamic Metadata Generation Logic for Terms
    const pageTitle = currentLang?.translations?.["page.terms.title"];
    const title = pageTitle ? `${pageTitle} - ${siteName}` : `Terms & Privacy - ${siteName}`;
        ...
        <div className="flex flex-col items-center w-full animate-fade-in-up mt-8 mb-16 px-4">
            <div className="w-full max-w-3xl">
                ...
                <div className="mt-8 text-center">
                    <Link href={`/${safeLocale}`} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-[var(--border-color)]/50 text-[var(--text-primary)] font-semibold hover:bg-[var(--border-color)] transition-all">
